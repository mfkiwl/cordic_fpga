...
assign rd_fifo_en=rnext && (read_index != C_M_AXI_BURST_LEN);//-1
	// Add user logic here
ddr2hdmi ddr2hdmi//从ddr读取
                (
                    .rst(~M_AXI_ARESETN),
                    .wr_clk(M_AXI_ACLK),
                    .rd_clk(video_clk),
                    .wr_data_count(wr_data_count),
                    .wr_en(rd_fifo_en),///(rnext && (read_index != C_M_AXI_BURST_LEN)),//axis_right(加一个判定坐标是否在范围内，不在范围内黑色)//(M_AXI_ARREADY && axi_arvalid),//(ddr_rd_en),//2022.01.05
 ...
 //判断整体系统上电后的第一帧的标志
    ...
    begin                                                                             
        ...
        else if(wr_over)//(x是用来判别的标志)//&&rd_load_flag)
        begin
            first_frame <= 1'b1;
            hdmi_start<=1'b1;
        end
